...
export default async function AnimationPage(props: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; warn?: string }> }) {
  ...
  const [impact, clients, products, brands, users] = await Promise.all([
    animationImpact(id),
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        ...
        <Card><div className="label">ROI animation</div><div className="kpi mt-2">{roi !== null ? `${roi.toFixed(1)}×` : "—"}</div><div className="mt-2 text-[12px] text-muted">coût {fmtMAD(cost)} · {anim.customersAdvised} clientes conseillées · {anim.samples} échantillons</div></Card>
      </div>
