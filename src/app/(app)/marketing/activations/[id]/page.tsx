        ...
        <div className="space-y-4">
          ...
          <Card title="Budget" action={<span className="text-[12px] text-muted">{t.planned > 0 ? `${Math.round(((t.committed + t.materials) / t.planned) * 100)} % engagé` : "Aucun prévu"}</span>}>
            ...
            <div className="mt-3 pt-3 border-t border-line">
              <div className="flex items-center justify-between"><span className="label">Matériel consommé (inventaire, valorisé)</span><Link href="/marketing/materiel" className="text-[11.5px] text-accent hover:underline">Inventaire</Link></div>
              {budget.materials.length > 0 ? (
                <ul className="mt-1.5 text-[12.5px] space-y-0.5">{budget.materials.map((m) => (
                  <li key={m.id} className="flex items-center gap-2"><span>{m.item} <span className="text-muted">× {m.quantity} {m.unit} · {m.category}</span></span><span className="tabular-nums ml-auto">{fmtMAD(m.total)}</span>
                    {canEdit && <form action={returnFromActivation}><input type="hidden" name="activationId" value={a.id} /><input type="hidden" name="materialId" value={m.id} /><button className="text-faint hover:text-red" type="submit" title="Retour en stock">×</button></form>}</li>
                ))}</ul>
              ) : <p className="mt-1 text-[12.5px] text-muted">Rien de sorti. Une sortie décrémente l&apos;inventaire et compte dans le coût complet.</p>}
              {canEdit && inventory.length > 0 && (
                <form action={consumeForActivation} className="mt-2 flex flex-wrap gap-1.5 text-[12.5px]">
                  <input type="hidden" name="activationId" value={a.id} />
                  <select name="itemId" className="select h-8 flex-1 min-w-[200px] text-[12px]">{inventory.filter((i) => i.stock > 0).map((i) => <option key={i.id} value={i.id}>{i.name}{i.brand ? ` · ${i.brand}` : ""} — {i.stock} {i.unit} dispo.</option>)}</select>
                  <input name="quantity" inputMode="numeric" className="input h-8 w-[90px] text-[12px]" placeholder="Qté" required />
                  <input type="date" name="date" defaultValue={todayIso} className="input h-8 w-auto text-[12px]" />
                  <button className="btn-secondary btn-sm h-8" type="submit">Sortir du stock</button>
                </form>
              )}
              {canEdit && inventory.length === 0 && <p className="mt-1 text-[11.5px] text-muted">L&apos;inventaire est vide : <Link href="/marketing/materiel" className="text-accent">créer ou importer des articles</Link>.</p>}
            </div>
          </Card>
          ...
          <Card title="Résultats" action={a.resultsAt ? <span className="text-[12px] text-muted">Saisis {fmtAgo(a.resultsAt)}</span> : <span className="text-[12px] text-muted">Rien d&apos;obligatoire</span>}>
            ...
            {(() => {
              const v = roiVerdict({ fullCost: t.fullCost, comparison: impact?.comparison ?? null, attributedRevenue: a.attributedRevenue, hasResults: a.hasResults, hasSalesScope: !!impact, unit: { contact: unitCost(t.fullCost, a.participants), sample: unitCost(t.fullCost, a.samples), pharmacy: unitCost(t.fullCost, a.pharmaciesReached) } }, settings.activations);
              const c = impact?.comparison ?? null;
              return (
                <div className="mt-3 pt-3 border-t border-line text-[12.5px]">
                  <div className="flex items-center gap-2 flex-wrap"><span className="label">Impact ventes (corrélation observée)</span><Badge tone={safeTone(VERDICT_TONES[v.verdict])}>{VERDICT_LABELS[v.verdict]}</Badge><Link href={`/marketing/activations/${a.id}/synthese`} className="ml-auto text-accent hover:underline">Fiche synthèse →</Link></div>
                  {!impact ? <p className="text-muted mt-1">Non mesurable : rattachez des points de vente ou une ville pour comparer les ventes avant / après.</p>
                    : <p className="text-muted mt-1">{impact.scopeLabel} · {c?.comparable ? `écart ${c.increment != null && c.increment >= 0 ? "+" : "−"}${fmtMAD(Math.abs(c?.increment ?? 0))}${v.roi != null ? ` (× ${v.roi.toFixed(2)} le coût)` : ""}` : `pas encore comparable (${c?.daysUntilComparable} j)`}</p>}
