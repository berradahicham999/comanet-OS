...
import { saveBudget, saveBudgetLine, deleteBudgetLine, saveExpense, deleteExpense, saveCampaign } from "./actions";

export const dynamic = "force-dynamic";
export const metadata = { title: "Marketing" };

const STATUS_LABEL = { PLANNED: "Prévu", COMMITTED: "Engagé", SPENT: "Dépensé" } as const;
const CAMPAIGN_STATUS = { DRAFT: ["gray", "Brouillon"], ACTIVE: ["green", "Active"], PAUSED: ["yellow", "En pause"], DONE: ["blue", "Terminée"] } as const;

export default async function MarketingPage(props: { searchParams: Promise<{ brand?: string; year?: string }> }) {
  ...
  type Campaign = { id: string; brand_id: string; name: string; channel: string; objective: string | null; start_date: string | null; end_date: string | null; budget: number | null; status: keyof typeof CAMPAIGN_STATUS; spend: number; revenue: number; conversions: number };
  ...
  return (
    <>
      <PageHeader eyebrow="Marketing Operating System" title="Budgets & campagnes" subtitle={`Année ${year} · budget prévu / engagé / dépensé / restant par marque, croisé avec le CA attribué et la marge.`}
        actions={<><Link href="/marketing/planning" className="btn-secondary btn-sm">Planning éditorial</Link><Link href="/imports?type=BUDGETS" className="btn-ghost btn-sm">Importer budgets</Link></>}>
        ...
        <Tabs current={brandId ? `/marketing?brand=${brandId}` : "/marketing"} tabs={[{ href: "/marketing", label: "Toutes les marques" }, ...activeBrands.map((b) => ({ href: `/marketing?brand=${b.id}`, label: b.name }))]} />
      </PageHeader>
            ...
            <section key={b.id} className="card overflow-hidden">
              ...
              <div className="grid lg:grid-cols-3 gap-4 p-4 sm:p-5">
                ...
                <div className="lg:col-span-2">
                  ...
                  <div className="label mt-4 mb-2">Campagnes ({camps.length})</div>
                  <div className="grid sm:grid-cols-2 gap-2">
                    {camps.map((c) => { const roas = c.spend ? c.revenue / c.spend : null; const cpa = c.conversions ? c.spend / c.conversions : null; const [tone, label] = CAMPAIGN_STATUS[c.status]; return (
                      <div key={c.id} className="rounded-xl border border-line p-3 text-[12.5px]">
                        <div className="flex items-center gap-2"><span className="font-medium flex-1 truncate">{c.name}</span><Badge tone={tone}>{label}</Badge><Badge tone="gray">{c.channel}</Badge></div>
                        <div className="text-muted mt-1">{c.objective ?? ""} {c.start_date && <span>· {fmtDateShort(c.start_date)} → {fmtDateShort(c.end_date)}</span>}</div>
                        <div className="mt-2 grid grid-cols-4 gap-1 text-center"><div><div className="text-[10px] text-muted">Budget</div><div className="font-medium">{c.budget ? fmtMAD(c.budget, { compact: true, suffix: false }) : "—"}</div></div><div><div className="text-[10px] text-muted">Dépensé</div><div className="font-medium">{fmtMAD(c.spend, { compact: true, suffix: false })}</div></div><div><div className="text-[10px] text-muted">ROAS</div><div className="font-medium">{roas !== null ? roas.toFixed(1) + "×" : "—"}</div></div><div><div className="text-[10px] text-muted">CPA</div><div className="font-medium">{cpa !== null ? fmtMAD(cpa, { suffix: false }) : "—"}</div></div></div>
                      </div>
                    ); })}
                  </div>
                  <details className="mt-2"><summary className="cursor-pointer text-[12px] text-accent font-medium">+ Nouvelle campagne</summary>
                    <form action={saveCampaign} className="mt-2 grid grid-cols-2 md:grid-cols-4 gap-1.5 text-[12px]">
                      <input type="hidden" name="brandId" value={b.id} />
                      <input name="name" placeholder="Nom de la campagne" className="input h-8 col-span-2" required />
                      <select name="channel" className="select h-8"><option value="META">Meta</option><option value="TIKTOK">TikTok</option><option value="GOOGLE">Google</option><option value="INFLUENCE">Influence</option><option value="TRADE">Trade</option><option value="EVENEMENT">Événement</option><option value="AUTRE">Autre</option></select>
                      <select name="status" className="select h-8"><option value="DRAFT">Brouillon</option><option value="ACTIVE">Active</option><option value="PAUSED">En pause</option><option value="DONE">Terminée</option></select>
                      <input name="objective" placeholder="Objectif" className="input h-8 col-span-2" />
                      <input type="date" name="startDate" className="input h-8" /><input type="date" name="endDate" className="input h-8" />
                      <input name="budget" placeholder="Budget MAD" className="input h-8" />
                      <button className="btn-secondary btn-sm col-span-2 md:col-span-3" type="submit">Créer</button>
                    </form>
                  </details>
                </div>
              </div>
