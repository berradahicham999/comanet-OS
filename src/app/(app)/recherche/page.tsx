...
export default async function RecherchePage(props: { searchParams: Promise<{ q?: string }> }) {
  ...
  const sections: Section[] = ([
    ...
    { title: "Fournisseurs", module: "achats", rows: (found.suppliers ?? []).map((h) => ({ id: h.id, label: h.label, sub: h.sub })), render: (r) => <Link href={`/gestion/fournisseurs/${r.id}`} className="card px-4 py-3 flex items-center gap-3 hover:border-line-2"><span className="font-medium flex-1 truncate">{r.label}</span><span className="text-[12px] text-muted">{r.sub}</span></Link> },
    { title: "Pièces d'achat", module: "achats", rows: (found.purchases ?? []).map((h) => ({ id: h.id, label: h.label, sub: h.sub })), render: (r) => <Link href={`/gestion/achats/${r.id}`} className="card px-4 py-3 flex items-center gap-3 hover:border-line-2"><span className="font-medium font-mono">{r.label}</span><span className="text-[12px] text-muted flex-1 truncate">{r.sub}</span></Link> },
