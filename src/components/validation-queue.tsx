...
export type QueueItem = {
  ...
  /** Ligne de contexte libre (activations : type, dates, ville) ; remplace plateforme / format. */
  meta?: string | null;
  /** Bloc de détail affiché sous le texte (activations : budget par poste, checklist). */
  details?: React.ReactNode;
};

export function ValidationQueue({ items, refs, initialId, validateKey, correctKey, action, canAct, nowIso, hrefBase = "/marketing/planning", fileHrefBase = "/marketing/planning/fichier", emptyVisualText, dateLabel = "publication le", actorLabel = "créé par", correctLabel = "Corriger", correctSubmitLabel = "Envoyer les corrections", correctPlaceholder = "Ce qui doit être corrigé (obligatoire, envoyé au créateur)…" }: {
  items: QueueItem[]; refs: Pick<ContentRefs, "platforms" | "formats">; initialId?: string | null; nowIso: string;
  /** Bases des liens vers la fiche et vers un fichier (par défaut : planning éditorial). Des chaînes, pas des fonctions : le composant est client. */
  hrefBase?: string; fileHrefBase?: string;
  emptyVisualText?: React.ReactNode; dateLabel?: string; actorLabel?: string; correctLabel?: string; correctSubmitLabel?: string; correctPlaceholder?: string;
  ...
  const href = (id: string) => `${hrefBase}/${id}`;
  const fileHref = (id: string) => `${fileHrefBase}/${id}`;
      ...
      <div className="card card-pad order-1 lg:order-2">
        <div className="flex items-start gap-2 flex-wrap">
          <div className="min-w-0 flex-1">
            <div className="text-[12px] text-muted flex items-center gap-1.5 flex-wrap"><BrandDot color={cur.color} />{cur.brand}{cur.meta ? <span>· {cur.meta}</span> : <>{pf && <span className="inline-flex items-center gap-1">· <PlatformIcon icon={pf.icon} label={pf.label} size={12} />{pf.label}</span>}{fmtLabel && <span>· {fmtLabel}</span>} · {dateLabel} {new Date(cur.date + "T12:00:00Z").toLocaleDateString("fr-FR")}</>}</div>
            <h2 className="font-semibold text-[16px] leading-tight mt-0.5"><Link href={href(cur.id)} className="hover:underline">{cur.title}</Link></h2>
            <div className="text-[11.5px] text-muted mt-0.5">En attente depuis {since <= 0 ? "aujourd'hui" : `${since} j`}{cur.responsible ? ` · ${actorLabel} ${cur.responsible}` : ""} · {idx + 1}/{items.length}</div>
          </div>
        ...
        <div className="mt-3 grid md:grid-cols-2 gap-3">
          <div className="rounded-xl border border-line bg-black/5 min-h-[220px] flex items-center justify-center overflow-hidden">
            {cur.assetId && cur.assetMime?.startsWith("image/") ? <img src={fileHref(cur.assetId)} alt={cur.assetName ?? ""} className="max-h-[480px] w-full object-contain" />
              : cur.assetId && cur.assetMime?.startsWith("video/") ? <video src={fileHref(cur.assetId)} controls className="max-h-[480px] w-full" />
              : cur.assetId ? <a href={fileHref(cur.assetId)} target="_blank" rel="noreferrer" className="text-[13px] text-accent underline p-4">{cur.assetName}</a>
              : <div className="text-[13px] text-muted p-4 text-center">{emptyVisualText ?? <>Aucun livrable déposé.<br />La légende seule est soumise.</>}</div>}
          </div>
          <div className="text-[13.5px] whitespace-pre-wrap leading-relaxed">
            {cur.caption ? cur.caption : <span className="text-muted">{cur.meta ? "Pas de description." : "Pas de légende."}</span>}
            {cur.hashtags && <div className="mt-2 text-accent">{cur.hashtags}</div>}
            {cur.details && <div className="mt-3 whitespace-normal">{cur.details}</div>}
          </div>
        </div>
          ...
          <div className="mt-4 sticky bottom-0 bg-surface pt-2 -mx-4 px-4 pb-1 sm:static sm:m-0 sm:p-0">
            {!correct ? (
              <div className="flex gap-2">
                ...
                <button type="button" className="btn-secondary flex-1 h-11 inline-flex items-center justify-center gap-2" disabled={pending || !correctKey} onClick={() => setCorrect(true)}><MessageSquareWarning size={16} /> {correctLabel} <kbd className="hidden sm:inline text-[10px] opacity-70 ml-1">C</kbd></button>
              </div>
            ) : (
              <div className="space-y-2">
                <textarea value={comment} onChange={(e) => setComment(e.target.value)} className="textarea min-h-[80px]" placeholder={correctPlaceholder} autoFocus />
                <div className="flex gap-2">
                  <button type="button" className="btn-primary h-10 flex-1" disabled={!comment.trim() || pending} onClick={() => submit(correctKey, comment)}>{correctSubmitLabel}</button>
            ...
            <p className="hidden sm:block text-[11px] text-faint mt-1.5">↑ ↓ ou J / K pour naviguer · V valider · C {correctLabel.toLowerCase()} · Échap annuler</p>
          </div>
        )}
