...
export type AssetOwner = { contentId: string } | { activationId: string } | { inventoryItemId: string };

export type AssetMeta = {
  id: string; contentId: string | null; activationId: string | null; inventoryItemId: string | null;
...
function ownerColumns(owner: AssetOwner): { contentId: string | null; activationId: string | null; inventoryItemId: string | null } {
  return {
...
export async function listAssets(owner: AssetOwner | string): Promise<AssetMeta[]> {
  const r = await db.execute<AssetMeta>(sql`
    select a.id, a.content_id as "contentId", a.activation_id as "activationId", a.inventory_item_id as "inventoryItemId",
...
export async function readAsset(id: string): Promise<{ name: string; mime: string; data: Buffer; contentId: string | null; activationId: string | null; inventoryItemId: string | null } | null> {
  const rows = await db.select({
    name: contentAssets.name, mime: contentAssets.mime, data: contentAssets.data,
    contentId: contentAssets.contentId, activationId: contentAssets.activationId, inventoryItemId: contentAssets.inventoryItemId,
  }).from(contentAssets).where(eq(contentAssets.id, id));
  const r = rows[0];
  return r?.data ? { name: r.name, mime: r.mime, data: r.data, contentId: r.contentId, activationId: r.activationId, inventoryItemId: r.inventoryItemId } : null;
}
