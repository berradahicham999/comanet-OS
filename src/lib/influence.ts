...
export async function listCollaborations(range: Range, filter?: { brandId?: string | null; influencerId?: string | null; status?: string | null; brandIds?: string[] | null }): Promise<CollabRow[]> {
  const res = await db.execute(sql`
    select c.id, c.date::text as date, c.influencer_id, i.name as influencer, i.followers, i.engagement_rate::float8 as engagement_rate,
           ...
           c.promo_code, c.conversions, c.attributed_revenue::float8 as attributed_revenue
 ...
 * il n'est jamais remplacé par une valeur inventée.
 */
...
export function rankInfluencers(rows: CollabKpis[], meta: Map<string, { instagram: string | null; tiktok: string | null; active: boolean; usualRate: number | null }>): InfluencerScore[] {
  const by = new Map<string, InfluencerScore & { _reach: number; _hasReach: boolean; _inter: number; _hasInter: boolean }>();
  for (const r of rows) {
    let e = by.get(r.influencer_id);
    if (!e) {
      const m = meta.get(r.influencer_id);
      e = {
        id: r.influencer_id, name: r.influencer, instagram: m?.instagram ?? null, tiktok: m?.tiktok ?? null,
        followers: r.followers, category: r.category, city: r.city, active: m?.active ?? true, usualRate: m?.usualRate ?? null,
        collabs: 0, cost: 0, reach: null, interactions: null, engagement: null, cpm: null,
        measuredRevenue: 0, measuredCost: 0, roas: null, score: null, lastDate: null, brands: [],
        _reach: 0, _hasReach: false, _inter: 0, _hasInter: false,
      };
      by.set(r.influencer_id, e);
    }
    e.collabs++;
    e.cost += r.cost;
    if (r.reach !== null) { e._reach += r.reach; e._hasReach = true; }
    if (r.interactions !== null) { e._inter += r.interactions; e._hasInter = true; }
    if (r.measured && r.attributed_revenue !== null) { e.measuredRevenue += r.attributed_revenue; e.measuredCost += r.cost; }
    if (!e.lastDate || r.date > e.lastDate) e.lastDate = r.date;
    if (!e.brands.includes(r.brand)) e.brands.push(r.brand);
  }
  const list = [...by.values()].map((e) => {
    const reach = e._hasReach ? e._reach : null;
    const inter = e._hasInter ? e._inter : null;
    return {
      ...e,
      reach,
      interactions: inter,
      engagement: reach && reach > 0 && inter !== null ? (inter / reach) * 100 : null,
      cpm: reach && reach > 0 ? (e.cost / reach) * 1000 : null,
      roas: e.measuredCost > 0 ? e.measuredRevenue / e.measuredCost : null,
    };
  });
  // score relatif, même logique que pour les collaborations
  const cpms = list.map((r) => r.cpm).filter((v): v is number => v !== null && v > 0);
  const engs = list.map((r) => r.engagement).filter((v): v is number => v !== null);
  const roass = list.map((r) => r.roas).filter((v): v is number => v !== null);
  const bestCpm = cpms.length ? Math.min(...cpms) : null;
  const maxEng = engs.length ? Math.max(...engs) : null;
  const maxRoas = roass.length ? Math.max(...roass) : null;
  ...
  scored.sort((a, b) => (b.score ?? -1) - (a.score ?? -1) || b.reach! - a.reach!);
  return scored.map(({ _reach, _hasReach, _inter, _hasInter, ...r }) => { void _reach; void _hasReach; void _inter; void _hasInter; return r; });
}
...
export function influenceAdvice(rows: CollabKpis[], ranked: InfluencerScore[]): { tone: "green" | "orange" | "red" | "blue"; title: string; detail: string }[] {
  ...
  if (weak.length) {
    out.push({
      ...
      detail: `${weak.slice(0, 3).map((r) => r.name).join(", ")} : coût par personne touchée nettement supérieur à la moyenne. Renégocier le tarif ou remplacer avant le prochain cycle.`,
    });
  }
  const late = rows.filter((r) => ["CONFIRMEE", "CONTENU_RECU"].includes(r.status) && r.date < new Date().toISOString().slice(0, 10));
  if (late.length) {
