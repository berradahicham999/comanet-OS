...
export type InventoryItemRow = {
  ...
  value: number; lastOutAt: string | null; lastInAt: string | null; outQty90: number; photoId: string | null;
...
export async function listInventory(f: { brand?: string | null; category?: string | null; q?: string | null; includeInactive?: boolean } = {}, todayIso: string, dormantDays: number): Promise<InventoryItemRow[]> {
  ...
  return r.rows.map((x) => ({ ...x, status: inventoryStatus({ stock: x.stock, alertThreshold: x.alertThreshold, lastOutAt: x.lastOutAt, createdAt: null }, todayIso, dormantDays) }));
}
