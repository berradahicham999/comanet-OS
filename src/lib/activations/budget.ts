...
export async function expectedExpenses(activationId: string): Promise<{ rows: ExpenseRow[]; planned: number; campaignId: string | null }> {
  const [a] = await db.select({ status: activations.status, brandId: activations.brandId, productId: activations.productId, date: activations.date, campaignId: activations.campaignId }).from(activations).where(eq(activations.id, activationId));
  if (!a) return { rows: [], planned: 0, campaignId: null };
  ...
  return { rows, planned: budgetTotals(lines, materials).planned, campaignId: a.campaignId };
}
...
export async function syncActivationExpenses(activationId: string): Promise<void> {
  const { rows, planned, campaignId } = await expectedExpenses(activationId);
  await db.transaction(async (tx) => {
    ...
    for (const r of rows) {
      await tx.execute(sql`
        insert into marketing_expenses (brand_id, activation_id, activation_ref, campaign_id, product_id, category, label, amount, status, date)
        values (${r.brandId}::uuid, ${activationId}::uuid, ${r.ref}, ${campaignId}::uuid, ${r.productId}::uuid, ${r.category}::budget_category, ${r.label}, ${r.amount}, ${r.status}::expense_status, ${r.date}::date)
        on conflict (activation_id, activation_ref) where activation_ref is not null do update set
          brand_id = excluded.brand_id, campaign_id = excluded.campaign_id, product_id = excluded.product_id, category = excluded.category, label = excluded.label,
