...
export async function ensureRegulatoryTasks() {
  ...
  const r = await db.execute(sql`
    select rf.id, rf.dossier, coalesce(rf.reference, p.name, rf.dossier) as dossier_label, rf.variant_type, rf.size,
           rf.expiry_date::text as expiry_date, rf.responsible_id, rf.brand_id, p.name as product_name, rf.status::text as status
    from regulatory_files rf left join products p on p.id = rf.product_id
    where rf.expiry_date is not null and rf.expiry_date <= ${limit}::date and not rf.blocked
  ...
  const rows = r.rows as { id: string; dossier: string; dossier_label: string; variant_type: string; size: string | null; expiry_date: string; responsible_id: string | null; brand_id: string | null; product_name: string | null; status: string }[];
  let created = 0;
  for (const row of rows) {
    ...
    await db.insert(tasks).values({
      title: `Redépôt DMP — ${row.dossier_label}${row.size ? " (" + row.size + ")" : ""}`,
      description: `Enregistrement ${daysLeft < 0 ? "expiré depuis " + Math.abs(daysLeft) + " jours" : "expirant dans " + daysLeft + " jours"} (validité ${row.expiry_date}). Tâche créée automatiquement par COMANET OS.`,
